...
# This is the same as switch_model.hawaii.hydrogen, but uses the MCET dates
# module (and tp_dates.csv) to group timepoints into dates. This ensures that
# all hydrogen is stored in a refrigerated tank if it is not used the same day
# it is produced.

# The `dates` module must be listed in modules.txt before this one.
...
def define_hydrogen_components(m):
    ...
    m.StoreLiquidHydrogenKg = Expression(
        ...
        rule=lambda m, z, d: sum(
            m.ts_duration_of_tp[m.tp_ts[tp]] * m.LiquifyHydrogenKgPerHour[z, tp]
