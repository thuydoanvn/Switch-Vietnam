...
def define_arguments(argparser):
    ...
    argparser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Charge a user defined Time-of-Use price for electricity, rather than varying hour by hour"
...
def define_components(m):
    ...
    #Place holder for ToU pricing: Se debe definir que cada sub set dentro del periodo tenga el mismo peso
    #Es decir, si haay tres bloque en el día, cada dia del perido debe tener el mismo peso en el bloque
    ...
    if m.options.dr_tou_pricing:
        ...
        m.tp_block = Param(m.TIMEPOINTS, within=m.BLOCK)
        m.min_data_check('BLOCK', 'tp_block')
        m.TPS_IN_BLOCK = Set(
            m.BLOCK,
            # ordered=True,
            initialize=lambda m, b: [tp for tp in m.TIMEPOINTS if m.tp_block[tp] == b],
        )

        # For each block, constrain all timepoints in the block to have the same bid weight
        def tou_bid_weight_rule(m, b, z, blk):
            tps = list(m.TPS_IN_BLOCK[blk])
            if len(tps) <= 1:
                return Constraint.Skip
            ref_tp = tps[0]
            # Only create constraints for tps[1:] (skip the reference)
            # This function will be called for each (b, z, blk, tp) if we change the indexing
            # So, we need to restructure the constraint definition
            return Constraint.Skip  # Placeholder, see below for new constraint definition

        # New: define constraint over (b, z, blk, tp) for tp in tps[1:]
        def tou_bid_weight_rule_fixed(m, b, z, blk, tp):
            tps = list(m.TPS_IN_BLOCK[blk])
            if tp not in tps:
                return Constraint.Skip
            if tp == tps[0]:
                return Constraint.Skip
            ref_tp = tps[0]
            return m.DRBidWeight[b, z, m.tp_ts[tp]] == m.DRBidWeight[b, z, m.tp_ts[ref_tp]]
        ...
        m.DR_ToU_Bid_Weight = Constraint(
            ...
            m.BLOCK,
            m.TIMEPOINTS,
            rule=lambda m, b, z, blk, tp: tou_bid_weight_rule_fixed(m, b, z, blk, tp)
        )
...
def pre_iterate(m):
    ...
    converged = (
        ...
        and (prev_cost - best_cost) / abs(prev_direct_cost) <= 0.001
    )
...
def calibrate_model(m):
    ...
    base_price = 83  # average retail price ($/MWh)
    m.base_data = [
        (
            z,
            ts,
...
def get_bids(m):
    ...
    for z in m.LOAD_ZONES:
        for ts in m.TIMESERIES:
            ...
            # import pdb; pdb.set_trace()
            if m.options.dr_flat_pricing or m.options.dr_tou_pricing: #Placeholder for TOU pricing
...
def find_tou_prices(m, marginal_costs, revenue_neutral):
    # calculate TOU prices for an imaginary load-serving entity (LSE) who
    # must break even in each load zone and period.
    # LSE buys at marginal cost, sells at TOU prices
    # this is like a transformation on the demand function, where we are
    # now  selling to the LSE rather than directly to the customers
    #
    # LSE iterates in sub-loop (scipy.optimize.newton) to find TOU price:
    # set price (e.g., simple average of MC or avg weighted by expected demand)
    # offer price to demand side
    # receive bids
    # calc revenue balance for LSE (q*price - q.MC)
    # if > 0: decrease price (q will go up across the board)
    # if < 0: increase price (q will go down across the board) 
    ...
    for z in m.LOAD_ZONES:
        for p in m.PERIODS:
            price_guess = []
            for blk in m.BLOCK:
                price_guess_block = value(
                    sum(
                    marginal_costs[z, ts]["energy"][i]
                    * electricity_demand(m, z, tp, "energy")
                    for ts in m.TS_IN_PERIOD[p]
                    for i, tp in enumerate(m.TPS_IN_TS[ts]) if tp in m.TPS_IN_BLOCK[blk]
                    )
                    / sum(
                        electricity_demand(m, z, tp, "energy") * m.tp_weight_in_year[tp]
                    for tp in m.TPS_IN_PERIOD[p] if tp in m.TPS_IN_BLOCK[blk]
                    )
                )
                price_guess.append(price_guess_block)
                
            if revenue_neutral:
                # find a TOU price that produces revenue equal to marginal costs
                tou_prices[z, p] = scipy.optimize.newton(
                    revenue_imbalance_tou, price_guess, args=(m, z, p, marginal_costs), maxiter=1000
                )
            else:
                # used in final round, when LSE is considered to have
                # bought the final constructed quantity at the final
                # marginal cost
                tou_prices[z, p] = price_guess

    # construct a collection of TOU prices with the right structure

    final_prices = {}
    block_list = list(m.BLOCK)
            ...
            for ts in m.TS_IN_PERIOD[p]:
                ...
                for prod in m.DR_PRODUCTS:
                    ...
                    for tp in m.TPS_IN_TS[ts]:
                        blk = m.tp_block[tp]
                        i = block_index[blk]
                        final_prices[z, ts][prod].append(tou_prices[z, p][i] if prod == "energy" else 0.0)
    return final_prices
    ...
    for ts in m.TS_IN_PERIOD[period]:
        ...
        for prod in m.DR_PRODUCTS:
            ...
            for i, block in enumerate(m.BLOCK):
                for tp in m.TPS_IN_TS[ts]:
                    if tp in m.TPS_IN_BLOCK[block]:
                        prices[prod].append(tou_price[i] if prod == "energy" else 0.0)
...
def load_inputs(mod, switch_data, inputs_dir):
    ...
    if mod.options.dr_tou_pricing:
        switch_data.load_aug(
            optional=True,
            filename=os.path.join(inputs_dir, "tou_blocks.csv"),
            format='set',
            set=mod.BLOCK,
        )
        switch_data.load_aug(
